...
def weighted_l1_loss(inputs, targets, weights=None):
    loss = F.l1_loss(inputs, targets, reduction='none').cuda()
    #print(loss.size())
    #print(weights.size())
    #print(loss)
    #print(weights)
    if weights is not None:
        loss *= weights.expand_as(loss).cuda()
...
def train(dataloader, model, criterion, optimizer, epoch, log): 
    ...
    for i, data_batch in enumerate(dataloader):
        ...
        rgb_f, label, ranges, videos,weights = data_batch #phase_f,
        ...
        out = model(rgb_f) #[phase_0,phase_1],
        loss= weighted_l1_loss(out, label_var,weights)
        loss.backward()
...
#    df = pd.DataFrame(columns = ['video','utterance',str(args.label_name)+'_target', str(args.label_name)+'_prediction'])
    ...
    for i, data_batch in enumerate(dataloader):
        ...
        with torch.no_grad():
            ...
            #phase_1 = Variable(phase_1.type('torch.FloatTensor').cuda())
            label_var = Variable(label.type('torch.FloatTensor').cuda())
        output = model(rgb_f) #[phase_0,phase_1], 
...
def test(dataloader, model, train_mean, train_std):
    ...
    for i, data_batch in tqdm(enumerate(dataloader)):
        rgb_f, label, ranges, names,weights = data_batch  #phase_f, 
        '''
        if(names[2]=='244_aligned'):
          break;
        '''
        #print(names)
        with torch.no_grad():
...
def main():
    ...
    feature_path = ['../scripts/Extracted_Features/train/resnet50_ferplus_dag_features_fps=30_pool5_7x7_s1','../scripts/Extracted_Features/affwild2_train_set']
    ...
    video_names = os.listdir(feature_path[0])
    video_names.extend(os.listdir(feature_path[1]))
    #print(len(os.listdir(feature_path[1])))
    ...
    if args.test:
        ...
        if args.cnn =='resnet50':
            feature_path_test = ['','../scripts/Extracted_Features/test2/resnet50_ferplus_dag_features_fps=30_pool5_7x7_s1']
        test_video_names = os.listdir(feature_path_test[1])
        test_root_path = ['','../../Affwild2/cropped_aligned']
        aa=['','']
        test_dataset = Face_Dataset(test_root_path, feature_path_test, aa, test_video_names, label_name, test_mode=True, length=args.length, stride  = args.length, num_phase=args.L)
        test_loader = torch.utils.data.DataLoader(
                       test_dataset,
                       batch_size=args.batch_size//2,  #args.batch_size//2
        ...
        for i in range(5):
            ...
            model = Two_Stream_RNN(args.hidden_units, fold_id=i, label_name=args.label_name, num_phase=args.L)   #change !!!!!!!!!!!
            checkpoint = torch.load(model_path)
            net_key = [key for key in checkpoint.keys() if (key !='epoch') and (key !='iter')][0]
            state_dict = checkpoint[net_key]
            ...
            for video_name in video_dict:
                ...
                cr=os.listdir('../scripts/Extracted_Features/test2/resnet50_ferplus_dag_features_fps=30_pool5_7x7_s1/'+video_name)
                cr.sort()
                cr=[int(x.split(".npy")[0]) for x in cr]
                if cr[-1] == len(predictions):
                    ...
                    new_predictions=np.zeros((cr[-1], 1))
                    prev_pred = -5
                    id=0
                    for j in range(1, cr[-1]+1 ):
                        if j not in cr:
                            new_predictions[j-1]=prev_pred
                        else:
                            new_predictions[j-1]=predictions[id]

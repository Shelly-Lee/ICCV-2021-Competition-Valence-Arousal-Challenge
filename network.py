...
class Two_Stream_RNN(nn.Module):
    def __init__(self, mlp_hidden_units=[2048, 256, 256], dropout=0.5, label_name = 'arousal', fold_id = 0, num_phase=12):
        ...
        '''
        self.transform = nn.Sequential(nn.Linear(256, 256), #512,256
                                      nn.ReLU(inplace=True),
                                      nn.BatchNorm1d(256),
                                      nn.Dropout(dropout))
    ...
    def load_model_weights(self, model, model_path):
        ...
        return model
    def forward(self, rgb_data): #, phase_data
        ...
        features = torch.cat([features_spatial], dim=-1)  #, features_temporal
...
if __name__ == "__main__":
    ...
    import os
    root_path = '../scripts/dir-to-aligned-face/tra/processed'
    feature_path = '../scripts/Extracted_Features/train/resnet50_ferplus_dag_features_fps=30_pool5_7x7_s1'
